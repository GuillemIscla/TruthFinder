...
trait World[W <: World[W]] {
  ...
  private def parseSentence(raw_sentence:String):Either[String, Sentence[W]] = {
    def parseSubject(speaker:String, raw_subject:String, directObject:State):Either[String, (Reference[State], Boolean)] = {
      ...
      raw_subject match {
        case "I am" =>
          Right(Name(speaker), true)
        case "Someone is" =>
          Right(NumberOfPeople(1, isExact = false), true)
        case "Everyone is" =>
          Right(Everyone, true)
        case "No one is" =>
          Right(NumberOfPeople(1, isExact = false), false)
        case "It is" =>
          directObject match {
            case ws:WorldState[W]@unchecked =>
              possibleWorldAspects(Some(ws)).headOption
                .fold[Either[String, (Reference[State], Boolean)]](Left(s"World is badly defined"))(s => Right(s, true))
            case _ =>
              Left(s"Sentence s'$raw_sentence' references a wrong world state")
          }
        case atLeastRegex(_, n) =>
          Right((NumberOfPeople(n.toInt, isExact = false), true))
        case exactlyRegex(_, n) =>
          Right((NumberOfPeople(n.toInt, isExact = true), true))
        case nameRegex(name, _) =>
          Right((Name(name), true))
      }
    }
    ...
    raw_sentence match {
      case sentenceRegex(speaker, raw_subject, maybeNot, raw_directObject) =>
        for{
          ...
          subjectParsed <- parseSubject(speaker, raw_subject, directObject)
          directObjectAffirmation = maybeNot != " not "
          (subject, sentenceAffirmation) = subjectParsed
