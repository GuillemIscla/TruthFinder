...
trait TestWorld extends World[TestWorld] {
  ...
  def possibleWorldStates(war:Option[WorldAspectReference[TestWorld, WorldState[TestWorld]]], conversation:List[Sentence] = List()):List[WorldState[TestWorld]] =
    war match {
  ...
  def possibleWorldAspects(ws:Option[WorldState[TestWorld]], conversation:List[Sentence] = List()):List[WorldAspectReference[TestWorld, WorldState[TestWorld]]] =
    ws match {
  ...
  def races(conversation:List[Sentence] = List()):List[Race] = List(
    TestRace("Race1"),
    TestRace("Race2"),
    TestTruthRace("Race1"),
    TestTruthRace("Race2"),
  ...
  case class TestRace(stringRefPrefix:String) extends Race {
    ...
    def canSay(truthPieces: List[TruthPiece[State]], text:List[Sentence], sentenceIndex:Int): Boolean => Boolean = _ => true
  }
  ...
  case class TestTruthRace(stringRefPrefix:String) extends Race {
    ...
    def canSay(truthPieces: List[TruthPiece[State]], text:List[Sentence], sentenceIndex:Int): Boolean => Boolean = b => b
  }
